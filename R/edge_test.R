...
#'
#' @param cont_matrix [n, m, c, 2] a contingency table for each edge, where each contingency
...
#' @return test_stat [n x m] the test statistic per edge.
#' @export
...
sg.bern.edge_test <- function(cont_matrix, test_stat="fisher") {
  ...
  test_stats <- array(NaN, dim=c(n, m))
  for (i in 1:n) {
    for (j in 1:m) {
      # compute given test statistic on the contingency matrix for that edge
      test_stats[i, j] <- do.call(tfunc, list(cont_matrix[i, j,,]))$estimate
    }
  }
