...
In our basic simulation, we will use 2 classes with 4x4 probability matrices. The probability matrix for each class will be identical, except for 1 signal vertex having all 3 edges in which the probability for class 1 will be .25, and the probability for class 2 will be .75. We will also choose 2 more edges to distribute this probability weighting for from the other 3 vertices. We expect that the subgraph should perfectly be chosen to be the signal vertex. 
...
n = 2  # 4 total edges since undirected

dim <- 4

p <- array(runif(n^2), dim=c(dim, dim))  # p is initially random
p[lower.tri(p, diag=FALSE)] <- 0
edges <- sample(which(upper.tri(p)), n, replace = FALSE)  # select 2 random edges
...
train <- sg.bern.subgraph_train(samp, Y, 4, coherent=1, tstat = "fisher")
...
test <- array(NaN, dim=c(dim, dim, ns*2))
