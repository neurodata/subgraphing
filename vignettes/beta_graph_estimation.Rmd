...
As we can see above, our average MSE drops off exponentially as the number of samples increases. This suggests that our sampling code is functioning properly.
...
We begin by plotting the estimated alpha, beta and compare to the true alpha, beta for 100 and 5000 observed samples respectively. Due to the fact that the method of moments produces a consistent estimator, we know that our estimated parameters should be closer to the true values with more samples.
...
multiplot(plotlist=params, layout=as.matrix(1:length(params), nrow=1, byrow=TRUE))
for (k in 1:length(n_vec)) {
  edge_distr_plots <- list()
  n <- n_vec[k]
  samp <- sample_beta_graph(alpha, beta, n)
...
As we can see, after 500 iterations, our estimate is much closer to the true alpha, beta than at only 100 iterations, as we would intuitively expect.
...
Due to consistency, we expect that as n increases we should get closer and closer approximations of alpha, beta. and as such the difference (frobenius norm of the difference) between the estimated and true alpha, beta should decline. 
