...
\begin{align}
  R_n^x &= \frac{1}{|\mathcal{S}|} \sum_{(u, v) \in \mathcal{S}}\mathbb{I}\left\{(u, v) \notin \mathcal{\hat{S}}\right\}
\end{align}
...
In our basic simulation, we will use 2 classes with 3x3 probability matrices. The probability matrix for each class will be identical, except for 4 randomly selected edges, in which the probability for class 1 will be .25, and the probability for class 2 will be .75. 
...
p1 <- p + rnorm(dim^2, mean=0, sd=.1); p2 <- p + rnorm(dim^2, mean=0, sd=.1)  # initialize p1 and p2 to the same array with some noise
...
results <- data.frame(n=c(), error=c(), mis_edge=c())
for (n in ns) {
  ...
  Y[1:n] <- 0
  Y[(n+1):(2*n)] <- 1
  sg.bern.xval_classifier(samp=samp, Y=Y, nedges=nes, tstat="fisher", coherent = FALSE, xval="loo")
}
```
