...
p1 <- p1 + t(p1) - diag(diag(p1)) + rnorm(dim^2, mean=0, sd=.1)
p2 <- p2 + t(p2) - diag(diag(p2)) + rnorm(dim^2, mean=0, sd=.1)
p1[p1 > 1] <- 1; p1[p1 < 0] <- 0; p2[p2 > 1] <- 1; p2[p2 < 0] <- 0
p[,,1] <- p1
p[,,2] <- p2
...
samp[,,1:ns] <- sg.bern.sample_graph(p[,,1], s=ns, rewire=.15)
samp[,,(ns+1):(2*ns)] <- sg.bern.sample_graph(p[,,2], s=ns, rewire=.15)
...
As we can see from the plot of edges in our theoretically optimal subgraph, the signal vertices are vertices 1 and 2. As we show below, however, using an incoherent estimator does not necessarily return such a favorable output.
...
As we can see, we correctly identify edge 1 as a signal vertex, but fail to identify vertex 2 and instead determine vertex 3 to be a signal vertex. Let's take a look at how our performance is in a full-simulation setting:
...
for (sim in 1:10) {
  ...
  p1 <- p1 + t(p1) - diag(diag(p1)) + rnorm(xdim^2, mean=0, sd=.1)
  p2 <- p2 + t(p2) - diag(diag(p2)) + rnorm(xdim^2, mean=0, sd=.1)
  p1[p1 > 1] <- 1; p1[p1 < 0] <- 0; p2[p2 > 1] <- 1; p2[p2 < 0] <- 0
  p[,,1] <- p1
  p[,,2] <- p2
  for (n in ns) {
    samp <- array(NaN, dim=c(xdim, ydim, n*2))
    samp[,,1:n] <- sg.bern.sample_graph(p[,,1], s=n, rewire=.2)
    samp[,,(n+1):(2*n)] <- sg.bern.sample_graph(p[,,2], s=n, rewire=.2)
    
    Y <-array(NaN, dim=c(n*2))
    Y[1:n] <- 0
    Y[(n+1):(2*n)] <- 1
    for (ne in nes) {
      class_res <- sg.bern.xval_classifier(samp=samp, Y=Y, nedge=2*ne, tstat="fisher", coherent = FALSE, xval="loo")
...
Our cross-validated error is not bad, however, our missed-edge rate is quite poor. 
